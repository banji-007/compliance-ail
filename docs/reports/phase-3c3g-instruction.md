...
`phase-3c3g.md`, under `docs/reports/`, committed and pushed before this session closes. Verdict per item, the reproduction before the fix, the broken-selector output for each falsifier, demonstration, enforcing test, mutation result, mapping, could-not-verify, CI run id. Verdict by the red team's own labels: R1 through R5 (R6 is closed out of band; cite `r6-headstate.md` rather than re-verdicting it), and C1 to C10, saying explicitly which were untested rather than holding.
