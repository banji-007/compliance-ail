...
| `docs/adr/0002-fastapi-immudb-proxy.md`, Decision/Consequences | `/audit` response shape description is stale (predates the five-state, `payload_state`, `profile` model) - **found, not fixed** | Out of scope for this pass's named items; see section 10, "could not verify" |
...
- `docs/adr/0002-fastapi-immudb-proxy.md`'s Decision and Consequences sections describe `/audit`'s response shape as a single `verified: true|false` boolean with no `payload_state`, `profile`, or five-state model - this predates several phases of `/audit`'s own evolution and is stale in a way beyond R2's named scope (the port-exposure contradiction only). Found, not fixed - a design/content change beyond what R2 states, and R2's own instruction is narrowly "fix the contradiction," not "rewrite the ADR." Flagged rather than silently left, per "any claim not in the mapping" - it is in the mapping (section 9), marked as found-but-out-of-scope.
