...
- **`GET /audit`'s `total` is a walk over the ledger, and its cost grows with the ledger forever (P3c3a-4, Phase 3c-3a).** The count comes from ImmuDB's `count` over the `tool_call:` prefix on every request. It is bounded by the ledger rather than by the page, it is sub-linear but unbounded, and the dashboard polls this route every 30 seconds per open tab, so the cost recurs at that rate indefinitely. Measured figures at 2k, 10k and 40k keys are in `docs/reports/phase-3c3a.md`. A maintained counter would replace this without changing the response contract, because the contract is the number and not how it was obtained; it is deliberately not in this phase.
- **The `/audit` page is not ordered by time, so `has_more` means more records exist behind it, never more recent ones (P3c3a-2, Phase 3c-3a).** The page is served in ImmuDB key order and `tool_call:` keys lead with `agent_id`, so a page returns the lexicographically-largest agent ids rather than the newest decisions - the open finding in `TODO.md`, which Phase 3c-3b addresses. `has_more` is stated in exactly those terms and there is deliberately no cursor to go with it: a cursor names a position in an ordering that is about to be replaced. One further limit, stated rather than left implied: `limit` bounds the decision scan, and the synthesized rows for orphaned write-ahead intents (D16) are appended after it, so a response can carry more rows than `limit` asked for and `len(entries)` can exceed `total`.
